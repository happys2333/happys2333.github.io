...
[Read in English]({% post_path en/NLP01 %})
...
下一篇 [Transformer 概述]({% post_path zh-CN/Transformer %}) 会解释注意力如何让不同位置交换信息，并用一个小型时序预测模型展示输入与输出的形状。先理解这一篇的数据流程，再学习新架构，更容易分清哪些问题来自数据，哪些来自模型。
